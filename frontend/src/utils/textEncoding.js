...
function mojibakeScore(value) {
  ...
  const unknown = (value.match(/\?/g)?.length || 0) * 8;
  return markers * 10 + replacement + unknown;
}

const PHRASE_REPAIRS = [
  {
    pattern: /Đã g.i y.u c..u h.y l.ch\.?\s*Vui l.ng ch. ph.ng kh.m x.c nh.n\.?/i,
    replacement: 'Đã gửi yêu cầu hủy lịch. Vui lòng chờ phòng khám xác nhận.'
  },
  {
    pattern: /C. l.ch kh.m m.i/i,
    replacement: 'Có lịch khám mới'
  },
  {
    pattern: /L.ch h.n đã đ..c x.c nh.n/i,
    replacement: 'Lịch hẹn đã được xác nhận'
  },
  {
    pattern: /H. s. kh.m b.nh đã đ..c c.p nh.t/i,
